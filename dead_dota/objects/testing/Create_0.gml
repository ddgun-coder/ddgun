...
function skillQ_exception(_type) {
	switch (_type) {
		...
		case spr_hat59:
		if (obj_clock_ui.item_num = 2) {
			if (a_cooltime <= 0) {
				if (item_index != -1) {
					if (mana >= 27) {
						prt_val_add(Val.mp, -27);
						item_table(clock_ui);
						clock_ui = 54;
						item_index = -1;
						...
						skill_say_time = 30;
							skill_say = "MP: " + string(mana) + " / 27(27)";
					}
					}
					else {
						skill_say_time = 40;
	...
	break;
		...
		if (a_cooltime <= 0) {
			if (stemina >= 80) {
				...
				skill_say_time = 30;
				skill_say = "스테미나: " + string(stemina) + " / 140(80)";
			}
		}
		break;
...
break;
		...
		case spr_hat64:
			...
			default:
				if (a_cooltime <= 0) {
					if (mana >= 59) {
						prt_val_add(Val.mp, -59);
						normal_play(sang_3_act1);
					}
					else {
						skill_say_time = 30;
						skill_say = "MP: " + string(mana) + " / 59(59)";
					}
				}
			break;
			}
