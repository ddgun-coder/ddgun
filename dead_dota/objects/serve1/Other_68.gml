	...
	switch (buffer_type) {
			...
			if (buff_size > 2) {
				...
				if(nu_team == global.team) {
					...
					with (testing) {
						...
						buffer_write(testing.buff_chat,buffer_u16,200);
						buffer_write(testing.buff_chat,buffer_bool,1);
						buffer_write(testing.buff_chat,buffer_u16,spr_apple3);
		...
		case 94:
			...
			if (testing.live = 1 and global.hat != spr_hat5) {
				if (testing.hp > 50)
				{
					...
					buffer_write(buff_chat,buffer_u8,80);
					buffer_write(buff_chat,buffer_u16, 200);
			...
			else
			{
				...
				with(testing) {
					...
					buffer_write(buff_chat, buffer_u8, 0);
					buffer_write(buff_chat, buffer_u8, serve.team_score[up] + 1);
				...
				buffer_write(buff_chat,buffer_u8, spr_hat5);
				buffer_write(buff_chat,buffer_u8, 0);
		...
		case 203:
			if (buff_size > 2) {
				...
				if (testing.level != 4) {
					if (global.team) {
						global.hat = spr_hat1 + bol_hat1;
					}
					else {
						global.hat = spr_hat1 + bol_hat2;
					}
					...
					buffer_write(buff_chat,buffer_u8,95);//95는 고자동기화
					buffer_write(buff_chat,buffer_u8, global.hat);
