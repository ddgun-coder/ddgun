...
if(attack_speed = 30){
	var idda =  instance_create_depth(x, y, -100, ski_skill_p);
	idda.team = team;
	idda.cid = cid;
	idda.cid_id = cid_id;
	idda.YA = YA;
			...
			if (global.hat = spr_hat74) {
				 if (keyboard_check_released(global.keyw)) {
					 if (testing.mana >= 40) {
						...
						server202_sound(ski_3_sound);
						server78_create_instace(ski_skill3_changer ,x , y);
					 }
					  else {
					  testing.skill_say_time = 30;
					  testing.skill_say = "MP: " + string(testing.mana) + " / 40(40)";
					}
				}
			}
		}
